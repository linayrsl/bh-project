    ...
    let apiResponsePromise = axios
      ...
      .catch((error) => {
        if (error && error.response && error.response.status === 400) {
          toast.error(
            t(
              "ToastNotifications.verificationNotification",
              "יש להזין קוד אימות בן 5 ספרות"
            )
          );
        } else if (error && error.response && error.response.status === 401) {
          toast.error(
            t(
              "ToastNotifications.verificationNotification2",
              "קוד אימות שגוי. נא נסה שוב."
            )
          );
        } else if (error && error.response && error.response.status === 404) {
          toast.error(
            t(
              "ToastNotifications.verificationNotification3",
              "קוד האימות לא קיים. נא נסה להירשם בשנית."
            )
          );
        } else {
          toast.error(
            t(
              "ToastNotifications.verificationNotification4",
              "לא הצלחנו לאמת פרטים אישיים. נא נסה להירשם מחדש."
            )
          );
        }
        ...
        <div className="verification-body page-content-container ">
          <div className="verification-message">
            <div className="verification-message1">
              <Trans i18nKey={"verificationPage.verificationMessage"}>
                {" "}
                בדקו את תיבת הדואר
                <span>
                  {{
                ...
                . בדקות הקרובות תתקבל הודעה
              </Trans>
            </div>
            <div className="verification-message2">
              <Trans i18nKey={"verificationPage.verificationMessage2"}>
          ...
          <div className="verification-form">
            ...
            >
              ...
              <div className="verification-message3">
                <Trans i18nKey={"verificationPage.verificationCodeMessage"}>
          ...
          <div className="verification-cta">
            <ProceedButton
              isTriggered={this.state.isFormSubmitted}
              disabled={
...
const VerificationPage = withAppConfig(
  withTranslation()(
    withRouter(
